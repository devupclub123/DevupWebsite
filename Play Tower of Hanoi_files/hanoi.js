...
function init(mode, rel) {
    ...
    s += wrap({ cls: "copyrt", style: "left:5px; bottom:3px" });
    s += "</div>";
    docInsert(s);
...
function doDrop(dropNo) {
    ...
    if (p != disk.pole) {
        ...
        if (okQ) {
            ...
            my.log +=
                parseInt(my.dragStt - my.logStt) / 1000 +
                ", " +
                parseInt(performance.now() - my.logStt) / 1000 +
                ", " +
                disk.n +
                ", " +
                disk.pole +
...
function successDo() {
    document.getElementById("success").innerHTML = "Hoàn thành!";
    my.log +=
        parseInt(performance.now() - my.logStt) / 1000 + ", " + "Success!" + "\n";
}
function moveNChg(n) {
    if (n == 1) {
        this.moveN++;
...
class Pop {
    constructor(id, yesStr, yesFunc, noStr, noFunc) {
        ...
        this.div.style =
            "position:absolute; left:-450px; top:10px; width:auto; padding: 5px; border-radius: 9px; background-color: #88aaff; box-shadow: 10px 10px 5px 0px rgba(40,40,40,0.75); transition: all linear 0.3s; opacity:0; text-align: center; ";
