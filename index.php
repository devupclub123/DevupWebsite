<!DOCTYPE html>
<html>
    <head>
        <meta charset="UTF-8">
        <title>FPT QN DEVUP WEBSITE </title>
        <link rel="stylesheet" type="text/css" href="style.css">  
        <style type="text/css">
            body
            {  
                background-color: rgb(255, 255, 255);;
                font-family: Arial, Helvetica, sans-serif;
            }
        </style>
    </head>

    <body>
        <div id = "main">
            <div id = "header">
                <div class="header__imgname">
                    <img style="height: 60px; object-fit: cover; margin-right: 8px;" src="./Picture/logo2.png" alt="">
                    <h2 class="header_title" style=" font-weight: 330;">DevUp Club
                    <span>
                        <h4 style=" font-weight: 330; font-size: small;">FPTU University of Technology</h4>
                    </span>
                    </h2>       
                    </div>
                <ul class="header-menu">
                    <li> <a href="">Home</a> </li>
                    <li> <a href="./recruitment.html">Recruitment</a> </li>
                    <li> <a href="">Contact</a> </li>
                    <li> <a href="./about.html">About us</a> </li>
                </ul>
            
            </div>
            
            <div id = "banner">
                <canvas id="canvas"></canvas>
                <div class="banner-content">
                    
                    <img class="logo-banner" src="./Picture/logoDevup-chinh.png" width="200" height="200" >
                    <p class = "text1-banner">Are you <span class="text11-banner">ready</span> to join us?<p>
                    <h1 class="text2-banner">DevUp Club</h1>
                    <h1 class="text3-banner">Happy new year 
                        <span class="text31-banner">2</span>
                        <span class="text32-banner">0</span>
                        <span class="text33-banner">2</span>
                        <span class="text34-banner">3</span>
                    </h1>
                    <h4 class="position-banner">
                    <img src="./Picture/position.jpg" alt="position_icon" width="20" height="20">
                    FPT AI QUY NHON UNIVERSITY
                    </h4>
                
                    <button class="apply-button">
                        <a style="text-decoration: none" href="https://www.google.com/"> <h3 class="apply-text">Apply here</h3></a>
                    </button>
                   
                </div>
                
                
            </div>
        </div>
<!-------------------------------------------------------------------------------------->
        <br><br><br>
        <div id ="community">
            <div class="left">
                <p class="para1">Join the community</p>
                <p class="para2">We help students gain more Knowledge-Experience through practice</p>
                <p  class="para3">DevUp is a club, a community, a family where all members share the same passion for coding. 
                    We want to make a playground for FPT student who want to try and develop their coding skills.
                 By practicing to gain more knowledge and experience, DevUp is always ready to provide necessary tools,
                  humans for you to run projects. Join us, you can gain more than what you imaginate.
                </p>
                <button class="visit-button">
                    <a href="https://www.google.com/"> Visit our page here</a>
                </button>
            </div>
            <div class="right">
                <img src="./Picture/background.jpg" alt="" width="100%", height="100%">
            </div>
        </div>
<!-- ------------------------------------------------------------------------------------>
         <div id="Event">

           <div class="Event-content">
            <h2 class="Event-title">projects and events</h2>
            <h2 class="Event-Recently">Recently
                <span class="Event-Recently-Event"> Events</span>
            </h2>
           </div>

           <div id="Event-list">
                <div class="Event-website">
                 <img src="./Picture/Event_website.jpg" alt="" width="100%">
                <p class="Event-website-title"> <a href=""> DevUp website project</a></p> 
                </div>
                <div class="Event-YouthCode">
                 <img src="./Picture/Event_Youthcode.png" alt="" width="60%">
                  <p class="Event-YouthCode-title"><a  href=""> YouthCode chanllenge</a> </p>
                </div>
                <div class="Event-LearningSeries">
                <img src="./Picture/Event_learning.jpg" alt="" width="100%">
                <p class="Event-LearningSeries-title"><a href="">LearningSeries </a> </p>
                </div>
           </div>
         </div>
<!-- ------------------------------------------------------------------------------------>
<div id="function"> 
    <div id="function-content">
        <h2 class="function-content-plan">Recruitment plan</h2>
        <h2 class="function-content-function">DevUp-FUQN's Function</h2>
        <h2 class="function-content-function2">We're looking for...</h2>
    </div>
    <div class="function-tab">
        <button id ="function-tab1" class="tablinks active">Development</button>
        <button id ="function-tab2" class="tablinks">external relation</button>
        <button id ="function-tab3" class="tablinks">marketing</button>
        <button id ="function-tab4" class="tablinks">event operation</button>
        <button id ="function-tab5" class="tablinks">fessior solution</button>
    </div>
   
    <div id="Development" class="tabcontent">
        <div class="Development-content">
            <div class="Development-content1">
                <h3>Master Of Ceremonies (MC)</h3>
                <p>Event flow guidance </p>
            </div>
            <div class="Development-content2">
                <h3>Content creator</h3>
                <p>Construct event's content, script, agenda</p>
            </div>
            <div class="Development-content3">
                <h3>Developer in Java/Web/Mobile App/Game</h3>
                <p>Flow guidance</p>
            </div>
            <div class="Development-content3">
                <h3>Outstreach Specialist</h3>
                <p>Interact with members and mentors</p>
            </div>
            <div class="Development-content3">
                <h3>Graphic designer/Photographer/Videographer</h3>
                <p>Flow guidance</p>
            </div>
        </div>
    </div>
   
    <div id="external relation" class="tabcontent">
        <div class="externalRelation-content">
            <div class="externalRelation-content1">
                <h3>Content 1</h3>
                <p>
                    HTML là chữ viết tắt của cụm từ HyperText Markup Language((Xem thêm tại 
                    http://vi.wikipedia.org/wiki/HTML)) (dịch là Ngôn ngữ đánh dấu siêu văn bản)
                    được sử dụng để tạo một trang web, trên một website có thể sẽ chứa nhiều trang
                    và mỗi trang được quy ra là một tài liệu HTML.
                </p>
            </div>
            <div class="externalRelation-content2">
                <h3>Content 2</h3>
                <p>
                    HTML là chữ viết tắt của cụm từ HyperText Markup Language((Xem thêm tại 
                    http://vi.wikipedia.org/wiki/HTML)) (dịch là Ngôn ngữ đánh dấu siêu văn bản)
                    được sử dụng để tạo một trang web, trên một website có thể sẽ chứa nhiều trang
                    và mỗi trang được quy ra là một tài liệu HTML.
                </p>
            </div>
            <div class="externalRelation-content3">
                <h3>Content 3</h3>
                <p>
                    HTML là chữ viết tắt của cụm từ HyperText Markup Language((Xem thêm tại 
                    http://vi.wikipedia.org/wiki/HTML)) (dịch là Ngôn ngữ đánh dấu siêu văn bản)
                    được sử dụng để tạo một trang web, trên một website có thể sẽ chứa nhiều trang
                    và mỗi trang được quy ra là một tài liệu HTML.
                </p>
            </div>
        </div>
    </div>
   
    <div id="marketing" class="tabcontent">
        <div class="marketing-content">
            <div class="marketing-content1">
                <h3>Content 1</h3>
                <p>
                    CSS là chữ viết tắt của Cascading Style Sheets, nó là một ngôn ngữ được sử dụng để
                    tìm và định dạng lại các phần tử được tạo ra bởi các ngôn ngữ đánh dấu (ví dụ như HTML).
                </p>
            </div>
            <div class="marketing-content2">
                <h3>Content 2</h3>
                <p>
                    CSS là chữ viết tắt của Cascading Style Sheets, nó là một ngôn ngữ được sử dụng để
                    tìm và định dạng lại các phần tử được tạo ra bởi các ngôn ngữ đánh dấu (ví dụ như HTML).
                </p>
            </div>
            <div class="marketing-content3">
                <h3>Content 3</h3>
                <p>
                    CSS là chữ viết tắt của Cascading Style Sheets, nó là một ngôn ngữ được sử dụng để
                    tìm và định dạng lại các phần tử được tạo ra bởi các ngôn ngữ đánh dấu (ví dụ như HTML).
                </p>
            </div>
        </div>
    </div>
    <div id="event operation" class="tabcontent">
        <div class="eventOperation-content">
            <h3>PHP 2</h3>
            <p>
                PHP (viết tắt hồi quy "PHP: Hypertext Preprocessor") là một ngôn ngữ lập 
                trình kịch bản hay một loại mã lệnh chủ yếu được dùng để phát triển các ứng
                dụng viết cho máy chủ, mã nguồn mở, dùng cho mục đích tổng quát. Nó rất
                thích hợp với web và có thể dễ dàng nhúng vào trang HTML..
            </p>
        </div>
            
  </div>
    <div id="fessior solution" class="tabcontent">
        <div class="fessiorSolution-content">
            <h3>PHP 3</h3>
            <p>
                HTML là chữ viết tắt của cụm từ HyperText Markup Language((Xem thêm tại 
                http://vi.wikipedia.org/wiki/HTML)) (dịch là Ngôn ngữ đánh dấu siêu văn bản)
                được sử dụng để tạo một trang web, trên một website có thể sẽ chứa nhiều trang
                và mỗi trang được quy ra là một tài liệu HTML.
            </p>
        </div>
        
    </div>
</div>
<!-- ------------------------------------------------------------------------------------>
<div id="Sponsors">
    <h3 class="Sponsors-slogan">Many thanks to </h3>
    <h2 class="Sponsors-title">our event sponsors</h2>
    <div class="Sponsors-list">
        <h3 class="Sponsor-1"> Event sponsor</h3>
        <h3 class="Sponsor-2"> In-kind sponsor </h3>
    </div>
</div>
<!-- ------------------------------------------------------------------------------------>
<footer id="Footer">
    <div class="Footer-navigation">
        <!--Logo&Name-->
        <div id="Logo-Slogan"">
                <img  class="Logo-img" src="./Picture/logo2.png" alt="">
                <div class="nav__imgname">
                    <h2 >DevUp Club</h2>
                    <h4 >FPTU University of Technology</h4>    
                </div>
                <div id="mail-input">
                    <p class="subcribe-email"> Subcribe and receive our lastest news, events </p>
                    <form action="index.php" method="POST">
                    <?php $userEmail = "" ?>
                    <input type="text" name="email" placeholder="Enter your email" required value="<?php echo $userEmail ?>">
                    <input type="submit" name="btn-submit" value="Submit">
                    <?php
                    if (isset($_POST['btn-submit'])) {
                        $userEmail = $_POST['email'];
                        if (filter_var($userEmail, FILTER_VALIDATE_EMAIL)) {
                            $subject = "Cảm ơn đã theo dõi chúng tôi - DevUp";
                            $message = "Cảm ơn đã theo dõi website câu lạc bộ của chúng tôi. Bạn sẽ luôn nhận được những cập nhật mới nhất từ chúng tôi. Và chúng tôi sẽ không chia sẽ hoặc bán thông tin của bạn tới bất kỳ ai.";
                            $sender = "From: devup@gmail.com";
                            if (mail($userEmail, $subject, $message, $sender)) {
                                ?>
                                <div class="alert success">Cảm ơn đã theo dõi.</div>
                                <?php
                                $userEmail = "";
                            } else {
                                ?>
                                <div class="alert error">Gửi email không thành công.</div>
                                <?php
                            }
                        } else {
                            ?>
                            <div class="alert error">Email không đúng định dạng!</div>
                            <?php
                        }
                    }
                    ?>
                </form>
            </div>
        </div>
    </div>    
    <div id="Contact-infor">
        <p>Contact</p>
        <br>
        <p>Devup@gmail.com</p>
    </div>
    <div id="Follow-infor"> 
        <p>Follow here</p>
        <br>
        <p>Facebook</p>
        <p>Discord</p>
        <p>Linkedln</p>
    </div>
</footer>

<script type="text/javascript" src="fireworks.js"> </script>
<!-- ------------------------------------------------------------------------------------>
</body>
    
<!-- ------------------------------------------------------------------------------------>
<!-- tao hieu ung tab khi nhan vao (copy tren mang chu t cũng chua hieu doan nay :((  )-->
<script type="text/javascript">
    var buttons = document.getElementsByClassName('tablinks');
    var contents = document.getElementsByClassName('tabcontent');
    function showContent(id){
        for (var i = 0; i < contents.length; i++) {
            contents[i].style.display = 'none';
        }
        var content = document.getElementById(id);
        content.style.display = 'block';
    }
    for (var i = 0; i < buttons.length; i++) {
        buttons[i].addEventListener("click", function(){
            var id = this.textContent;
            for (var i = 0; i < buttons.length; i++) {
                buttons[i].classList.remove("active");
            }
            {this.className += " active";        }
            showContent(id);
        });
    }
    showContent('Development');
</script>
<!-- ------------------------------------------------------------------------------------>
</html>
